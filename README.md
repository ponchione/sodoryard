...
- Spec 23 prompt metadata work now keeps all checked-in built-in role prompts and embedded prompt assets synced with frontmatter for role key, persona, expected configured tools, receipt schema, recommended max turns, and structured-finding expectations. The metadata remains validation/documentation only; runtime tool registration still comes from `yard.yaml`.
