...
Spec 24's Electron desktop MVP has reached the current polish stop point. The
dev target starts the Vite renderer and opens Electron against a local
`yard serve --dev` backend, spawning the backend when one is not already
reachable:
