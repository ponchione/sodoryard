...
func renderYardChainMetrics(out io.Writer, report operator.ChainMetricsReport) {
	...
	_, _ = fmt.Fprintf(out, "findings events=%d open=%d addressed=%d open_ids=%s\n", report.ReceiptFindingEvents, report.OpenFindingCount, report.AddressedFindingCount, strings.Join(report.OpenFindingIDs, ","))
