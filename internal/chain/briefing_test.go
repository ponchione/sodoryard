...
func TestBuildStepBriefingIncludesLifecycleAndGuardrailFacts(t *testing.T) {
	briefing := BuildStepBriefing(StepBriefingInput{
		...
		Events: []Event{
			...
			{ID: 5, StepID: "resolve-2", EventType: EventStepGuardrailFacts, EventData: `{"role":"resolver","sequence":4,"receipt_valid":true,"changed_file_manifest_present":true,"changed_file_count":1,"source_writer_lock_release_attempted":true,"source_writer_lock_released":true,"addressed_ids":["FIND-correctness-001"]}`},
		},
	...
	for _, want := range []string{
		...
		"step 4 role=resolver receipt_valid=true manifest=true changed=1 lock_release_attempted=true lock_released=true addressed=FIND-correctness-001",
		"Open audit findings:",
