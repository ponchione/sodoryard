...
func TestChainRenderShowsGuardrailWarnings(t *testing.T) {
	...
	view := got.View()
	for _, want := range []string{"health: attention", "Warnings", "flow: chain completed after coder step 1 without later auditor", "Guardrails", "open=FIND-correctness-001", "finding id=FIND-correctness-001 source=correctness-auditor", "status=addressed addressed=2", "severity=high", "validation=rtk make test", "source_writer_lock acquired=1 released=1", "internal/example.go", "receipt_valid=true", "claim_matches=true", "code_index_dirty=true", "code_index_mark_supported=true", "code_index_mark_attempted=true", "code_index_marked=true", "brain_index_dirty=true"} {
		if !strings.Contains(view, want) {
