...
func TestChainRenderShowsGuardrailWarnings(t *testing.T) {
	...
	fake.details["chain-1"] = operator.ChainDetail{
		...
		Guardrails: operator.ChainGuardrailDetails{
			...
			StepFacts: []operator.StepGuardrailFactSummary{
				{StepID: "step-1", SequenceNum: 1, Role: "coder", ReceiptValid: true, ChangedFileManifestPresent: true, ChangedFileCount: 1, ChangedFileClaimPresent: true, ClaimedChangedFiles: []string{"internal/example.go"}, ChangedFileClaimMatchesManifest: true, CodeIndexDirty: true, CodeIndexDirtyMarked: true, BrainIndexDirty: true, SourceWriterLockReleased: true},
			},
		},
	...
	view := got.View()
	for _, want := range []string{"health: attention", "Warnings", "flow: chain completed after coder step 1 without later auditor", "Guardrails", "open=FIND-correctness-001", "finding id=FIND-correctness-001 source=correctness-auditor", "status=addressed addressed=2", "severity=high", "validation=rtk make test", "source_writer_lock acquired=1 released=1", "internal/example.go", "receipt_valid=true", "claim_matches=true", "code_index_dirty=true", "code_index_marked=true", "brain_index_dirty=true"} {
		if !strings.Contains(view, want) {
