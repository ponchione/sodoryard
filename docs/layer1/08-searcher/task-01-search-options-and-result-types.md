...
- [ ] File `internal/codeintel/searcher/types.go` created in the searcher package
- [ ] `SearchOptions` struct defined with the following fields and YAML/JSON tags:
  - `TopK int` — number of results per query (default: 10)
  - `Filters codeintel.Filter` — optional metadata filters, using the `codeintel.Filter` type (from L1-E01): `Language string` (empty = all languages), `ChunkType ChunkType` (empty = all types), `FilePathPrefix string` (empty = no path filter)
...
- [ ] `rankedResult` internal struct (unexported) defined for use by the dedup/re-rank logic:
  - `Chunk codeintel.Chunk` — the chunk data
...
- [ ] All types compile cleanly with `go build ./internal/codeintel/searcher/...`
