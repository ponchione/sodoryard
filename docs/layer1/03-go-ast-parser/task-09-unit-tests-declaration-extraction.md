...
Create a test Go project fixture and unit tests that verify declaration extraction produces correct `RawChunk`s for all supported Go construct types. The test fixture is a small, self-contained Go module under `internal/rag/goparser/testdata/` that contains representative declarations.

## Acceptance Criteria

- [ ] Test fixture directory: `internal/rag/goparser/testdata/testproject/` containing a valid Go module with `go.mod` and at least two packages
...
- [ ] Test: `TestExtractFunctionDeclaration` — verifies `PublicFunc` produces a `RawChunk` with:
  - `ChunkType` == `rag.Function` ("function")
  - `Name` == `"PublicFunc"`
...
- [ ] Test: `TestExtractInterfaceDeclaration` — verifies `SampleInterface` produces a `RawChunk` with:
  - `ChunkType` == `rag.Interface` ("interface")
...
- [ ] Test: `TestBodyTruncation` — creates or uses a declaration whose body exceeds 2000 characters and verifies the `Body` field is truncated to `rag.MaxBodyLength`
