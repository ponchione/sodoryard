...
Display the turn analyzer's detected signals and the semantic queries derived from the user's message. The signals section shows what the analyzer detected in the user's input (file references, symbol references, intent verbs, momentum patterns). The semantic queries section shows the queries that were sent to the RAG pipeline for retrieval. Together, these sections answer the question: "What did the system understand from my message, and what did it search for?"
