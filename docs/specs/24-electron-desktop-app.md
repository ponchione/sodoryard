...
- Chain detail includes desktop pause/resume/cancel controls with exact chain-id confirmation before cancellation, links timeline entries with conversation/turn metadata to a desktop context-report route, and renders receipt previews with frontmatter plus backend-validated changed-file open/reveal actions. Receipts have a desktop detail route for frontmatter, body, chain event review, receipt path copy, backend-validated source-spec and changed-file open/reveal actions, and step links back to chain detail anchors.
