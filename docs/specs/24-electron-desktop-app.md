...
- The settings route now shows project status, desktop app/backend version and launch-mode metadata, backend-validated runtime routing controls for the default provider/model, read-only fallback and agent settings, provider model metadata, provider credential status/remediation, and diagnostics export through `/api/diagnostics/export`.
